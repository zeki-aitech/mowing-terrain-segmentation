# DeepLabV3 with ResNet-50 backbone on YCOR dataset - Weighted Average Strategy
# Strategy: 25% Direct + 35% Square Root + 40% Log Inverse
...
# Weighted Average: 25% Direct + 35% Square Root + 40% Log Inverse
...
# Model configuration with weighted average strategy
model = dict(
    ...
    decode_head=dict(
        ...
        loss_decode=dict(
            ...
            use_sigmoid=False, 
            loss_weight=1.0,
            class_weight=weighted_avg_weights,
            avg_non_ignore=True,
        )
    ),  
    auxiliary_head=dict(
