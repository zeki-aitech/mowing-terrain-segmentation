# DeepLabV3 with ResNet-50 backbone on YCOR dataset - Capped Conservative Strategy
# Strategy: Log inverse weights capped at 8.0 to prevent extreme weights
...
# Capped Conservative: Log inverse weights capped at 8.0
...
# Model configuration with capped conservative strategy
model = dict(
    ...
    decode_head=dict(
        ...
        loss_decode=dict(
            ...
            use_sigmoid=False, 
            loss_weight=1.0,
            class_weight=capped_conservative_weights,
            avg_non_ignore=True,
        )
    ),  
    auxiliary_head=dict(
